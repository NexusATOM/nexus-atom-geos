...
def main():
    version("nexus-atom-geos")
    try:
        version("nexus-geos-agent")
    except PackageNotFoundError:
        pass
    else:
        raise RuntimeError("Old standalone distribution must not be co-installed")
    if find_spec("nexus_atom_controller") is not None:
        raise RuntimeError("Run this smoke check in the minimal environment without Controller")
