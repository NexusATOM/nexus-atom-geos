...
From this repository in its own environment:

```bash
uv sync --locked --extra dev
uv run python examples/run_agents_demo.py
```
...
```bash
uv run geos-agent investigate 'Trace pressure_log' \
  --workspace examples/demo/workspace.yaml --repo fvdycore --offline
```
...
```bash
uv run geos-agent gpu-port 'Assess a CUDA port of pressure_log' \
  --workspace examples/demo/workspace.yaml --repo fvdycore \
...
uv run geos-agent run MAPL unit-tests --workspace workspace.local.yaml
uv run geos-agent run MAPL unit-tests --workspace workspace.local.yaml --execute
```
...
`uv run python examples/run_engineering_demo.py` is the working no-key example of
that complete lifecycle. See [usage](USAGE.md) for `work`, patch and benchmark
commands, and [site integration](SITE-INTEGRATION.md) for GEOS wrappers.
...
uv run geos-agent resume SESSION_ID --status-only
uv run geos-agent session verify SESSION_ID
```
...
[sessions](SESSIONS.md) for dependency flags, model selection, recovery, and
...
Use the standalone tools as Python/CLI integration points, or the ATOM GEOS
plugin when adopting the shared capability protocol. Keep standalone and ATOM
GEOS installations in separate environments because they share package and CLI
names. There is currently no tested bridge to another team's orchestrator or
automatic migration between their state stores. See [readiness](READINESS.md).
